#This will create an IAM user for CircleCI to use for accessing the AWS ECR to push the docker image. 
#So create user, give it keys, and add a policy document to access ECR. Then take the user, keys, and 
#ECR url and then place it in a CircleCi context.

#Get aws account id. Current will use the aws account currently logged in
...
resource "aws_iam_access_key" "aws_access_key" {
  user = aws_iam_user.ci_wordpress
